...
type User struct {
	...
	Telephone      string         `gorm:"type:varchar(15)" rql:"filter" json:"telephone"`
	...
	CodeRecovery   string         `gorm:"type:varchar(5)" rql:"filter" json:"code_recovery"`
...
func (u *User) FindByID() (error, DataUser) {
	...
	if err != nil {
		utils.LogMessage{Title: "[MODELS>USER] Error on *User.FindByID()", Body: err.Error()}.Error()
	}

	if err = database.CloseConnection(db); err != nil {
		utils.LogMessage{Title: "[MODELS>USER] Error on database.CloseConnection(db) > *User.FindByID()", Body: err.Error()}.Error()
	}
	return err, dataUser
}
...
func (u *User) UpdateByColumn(column, newValue string) error {
	...
	if err != nil {
		utils.LogMessage{Title: "[MODELS>USER] Error on *User.UpdateByColumn()", Body: err.Error()}.Error()
	}

	if err = database.CloseConnection(db); err != nil {
		utils.LogMessage{Title: "[MODELS>USER] Error on database.CloseConnection(db) > *User.UpdateByColumn()", Body: err.Error()}.Error()
	}
	return err
}
